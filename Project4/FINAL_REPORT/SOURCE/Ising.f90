program Ising
  ...
  integer:: NumSpins,NumTemp,MCS,i,j,orientation,accept,acceptance
  ...
  ! Call initializer for random function seed
  call init_random_seed()

  ! Decide whether or initialze with random spins or ordered spins
  ...
  ! Loop through desired energies
  do i=1, NumTemp
     Temp = Temp + TempStep
     E=0.d0;
     M=0.d0;

     !Precalculate 5 allowed energy differences
     do j=-8,8
        w(j)=0.d0
     end do
     ...
     ! Initialze the spin matrix
     call initialize(NumSpins,Temp,SpinMatrix,E,M)

     !Call Metropolis algoritm for each Monte Carlo Cycle and update expectation values.
     do j=1, MCS
        call Metropolis(NumSpins, SpinMatrix, E,M,w,accept)
     ...
     !output energy for each temperature
...
subroutine Metropolis(NumSpins, SpinMatrix, E, M, w,accept)
  implicit none

  integer::NumSpins,x,y,ix,iy,deltaE,right,left,up,down,accept
  integer, dimension(NumSpins,NumSpins)::SpinMatrix
  real(8)::E,M
  real(8)::w(-8:8)
  accept=0
  !Place yourself on energy grid
  do y=1,NumSpins
     do x=1,NumSpins
...
!Results are normalized to number or spins and MCS
